...
exports.METADATA_NAME = "metadata";
class Multicore extends events_1.EventEmitter {
    constructor(key, store) {
        super();
        this.key = key;
        this.store = store;
        this.handlers = {};
        this.metadataHandler = new ydoc_1.default(exports.METADATA_NAME);
        this.activeFeeds = new Map();
        this.addHandler(this.metadataHandler);
        this.metadata = new metadata_1.default(this.metadataHandler.doc);
        const rootKey = this.key.toString("hex");
        const writerOf = new Set();
        this.loadFeed(exports.METADATA_NAME, rootKey);
    ...
    static create(store) {
        return new Promise((resolve) => {
            store.ready(() => {
                resolve(new Multicore(store.namespace(exports.METADATA_NAME).default().key, store));
            });
        });
    }
    static load(key, store) {
        return new Promise((resolve) => {
            store.ready(() => {
    ...
    get isAdmin() {
        return this.isWriter(exports.METADATA_NAME);
    }
    isWriter(kind) {
        const feeds = this.metadata.getFeeds(kind);
        if (!feeds) {
            ...
            feed.ready(() => {
                ...
                if (feed.key.equals(this.key)) {
                    ...
                    if (feed.writable && feed.length === 0) {
                        this.metadata.addFeed(exports.METADATA_NAME, key);
                    }
                    else {
