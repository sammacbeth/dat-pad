...
import { Awareness, encodeAwarenessUpdate, applyAwarenessUpdate, } from "y-protocols";
const messageQueryAwareness = 3;
const messageAwareness = 1;
export default class AwarenessExtension {
    constructor(doc, multicore, awareness = new Awareness(doc)) {
        ...
        const _awarenessChangeHandler = ({ added, updated, removed }) => {
            ...
            encoding.writeVarUint8Array(encoderAwareness, encodeAwarenessUpdate(this.awareness, changedClients));
        ...
        const _syncPeerHandler = (peer) => {
            ...
            if (awarenessStates.size > 0) {
                const encoder = encoding.createEncoder();
                encoding.writeVarUint(encoder, messageAwareness);
                encoding.writeVarUint8Array(encoder, encodeAwarenessUpdate(awareness, Array.from(awarenessStates.keys())));
                self.ext.send(Buffer.from(encoding.toUint8Array(encoder)), peer);
            }
        };
